...
/* USER CODE BEGIN PV */
//float Kp,Kd,control;
float alpha,T,control,oldcontrol,olderror;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  /* USER CODE BEGIN Callback 0 */
	  ...
	  if (htim->Instance == TIM10) {
	    ...
	    //tustin
	    control=(1/((1/(float)sampling_frequency)+2*alpha*T))*(
		((1/(float)sampling_frequency)+2*T)*(float)(ref-y)
		+((1/(float)sampling_frequency)-2*T)*olderror
		+(2*alpha*T-(1/(float)sampling_frequency))*oldcontrol );
	    //오일러
	    //control=((T*(float)(ref-y))+(((1/(float)sampling_frequency)-T)*olderror)+((alpha*T-(1/(float)sampling_frequency))*oldcontrol))*(1/(alpha*T));
	    //control = Kp*(float)(ref-y)-(float)sampling_frequency*Kd*(float)(y-oldy);
	    oldy=y;
	    olderror=(float)(ref-y);
	    oldcontrol=control;
