...
/* USER CODE BEGIN PV */
float K1,K2,control,L1,L2,Ts,hx1,hx2,oldhx1,oldhx2;
volatile int32_t x1,x2,ref,interrupt_counter,sampling_frequency,data_counter,oldx1;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  /* USER CODE BEGIN Callback 0 */
	  ...
	  if (htim->Instance == TIM10) {
	    ...
	    hx1=(float)oldhx1+Ts*(-10*(float)oldhx2+L1*(float)(oldx1-oldhx1));
	    hx2=(float)oldhx2+Ts*(-10*(K1*(float)ref-K1*(float)oldhx1-K2*(float)oldhx2)+L2*(float)(oldx1-oldhx1));
	    control=K1*(float)ref-K1*(float)hx1-K2*(float)hx2;
	    oldx1=x1;
	    oldhx1=hx1;
	    oldhx2=hx2;
